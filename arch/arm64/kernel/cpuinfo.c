...
static ssize_t show_cpuid(struct device *dev, struct device_attribute *attr, char *buf)
{
...
static DEVICE_ATTR(cpuid, 0444, show_cpuid, NULL);
...
static int __init cpuinfo_cpuid_sysfs_init(void)
{
	...
	for_each_present_cpu(cpu) {
		...
		ret = sysfs_create_group(&dev->kobj, &cpuid_attr_group);
		if (ret)
			break;
	}
	...
	for_each_present_cpu(cpu) {
		...
		dev = get_cpu_device(cpu);
		if (dev)
			sysfs_remove_group(&dev->kobj, &cpuid_attr_group);
	}

	return ret;
}

device_initcall(cpuinfo_cpuid_sysfs_init);
