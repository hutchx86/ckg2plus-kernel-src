			   ...
			   void *ed)
{
	...
	if (!r) {
		pr_warn("ARM64 CPU ERP: Could not find <%s> IRQ property. Proceeding anyway.\n",
			propname);
		return -EINVAL;
	}
	...
	if (rc) {
		pr_warn("ARM64 CPU ERP: Failed to request IRQ %d: %d (%s / %s). Proceeding anyway.\n",
...
static int arm64_cpu_erp_probe(struct platform_device *pdev)
{
	...
	if (IS_ENABLED(CONFIG_EDAC_CORTEX_ARM64_DBE_IRQ_ONLY)) {
		pr_warn("ARM64 CPU ERP: SBE detection is disabled.\n");
		goto out_irq;
	}
