				   ...
				   u32 *flags)
{
	...
	if (WARN_ON(gpio_chip->of_gpio_n_cells < 2)) {
		dev_err(gpio_chip->dev, "of_gpio_n_cells < 2\n");
		return -EINVAL;
	}

	q_spec = qpnp_pmic_pin_get_spec(q_chip, gpio_spec->args[0]);
	if (!q_spec) {
		dev_warn(gpio_chip->dev, "no such PMIC gpio %u in device topology\n",
							gpio_spec->args[0]);
		return -EINVAL;
	}
...
static int qpnp_pin_probe(struct spmi_device *spmi)
{
	...
	if (highest_gpio < lowest_gpio) {
		dev_warn(&spmi->dev, "%s: no device nodes specified in topology\n",
								__func__);
		rc = -EINVAL;
		goto err_probe;
