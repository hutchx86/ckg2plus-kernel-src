...
{
	if (!pon->pon_input) {
		pon->pon_input = devm_input_allocate_device(&pon->spmi->dev);
		if (!pon->pon_input) {
			dev_err(&pon->spmi->dev,
		...
		pon->pon_input->dev.parent = &pon->spmi->dev;
		pon->pon_input->id.bustype = BUS_HOST;
	}
...
static int qpnp_pon_reboot(struct notifier_block *nb,
			   unsigned long code, void *unused)
{
	struct qpnp_pon *pon =
		container_of(nb, struct qpnp_pon, reboot_notifier);

	if (code == SYS_HALT || code == SYS_POWER_OFF) {
		int rc, i;
		rc = qpnp_pon_masked_write(pon, QPNP_PON_KPDPWR_S1_TIMER(pon), QPNP_PON_S1_TIMER_MASK, 4);
		if (rc) {
			dev_warn(&pon->spmi->dev, "Unable to reconfigure S1 timer\n");
		}

		i = ilog2(5 + 1);
		rc = qpnp_pon_masked_write(pon, QPNP_PON_KPDPWR_S2_TIMER(pon), QPNP_PON_S2_TIMER_MASK, i);
		if (rc) {
			dev_warn(&pon->spmi->dev, "Unable to reconfigure S2 timer\n");
		}
	}
...
static int qpnp_pon_probe(struct spmi_device *spmi)
{
	...
	pon->reboot_notifier.notifier_call = qpnp_pon_reboot;
	register_reboot_notifier(&pon->reboot_notifier);
...
static int qpnp_pon_remove(struct spmi_device *spmi)
{
	...
	unregister_reboot_notifier(&pon->reboot_notifier);
