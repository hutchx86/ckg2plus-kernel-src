						...
						int last_client)
{
	...
	pr_debug("smbchg_hvdcp_enable_cb  enable %d last_enable %d\n",
			enable, last_enable);
	rc = smbchg_sec_masked_write(chip,
				chip->usb_chgpth_base + CHGPTH_CFG,
...
#define PCC_MASK                SMB_MASK(1, 0)
#define PCC_CFG                 0xF1
#define DEFAULT_PCC_MA                  100
...
static int smbchg_hw_init(struct smbchg_chip *chip)
{
	...
	rc = smbchg_sec_masked_write(chip, chip->chgr_base + PCC_CFG,
			PCC_MASK,
			(chip->cfg_prechg_current_ma-100)/50);
	if (rc < 0)
		dev_err(chip->dev, "Couldn't set PCC current rc=%d\n",
			rc);
...
static int smb_parse_dt(struct smbchg_chip *chip)
{
	...
	OF_PROP_READ(chip, chip->cfg_prechg_current_ma,
			"prechg-current-ma", rc, 1);
	if (chip->cfg_prechg_current_ma == -EINVAL)
		chip->cfg_prechg_current_ma = DEFAULT_PCC_MA;
...
static int smbchg_request_irqs(struct smbchg_chip *chip)
{
	...
	spmi_for_each_container_dev(spmi_resource, chip->spmi) {
		...
		if (!(resource && resource->start)) {
			dev_warn(chip->dev, "node %s IO resource absent!\n",
