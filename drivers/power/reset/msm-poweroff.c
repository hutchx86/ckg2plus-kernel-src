...
static void do_msm_poweroff_warm(void)
{
	pr_notice("X Powering off the SoC\n");
...
static int msm_reboot_notifier(struct notifier_block *nb,
...
static struct notifier_block msm_reboot_nb = {
	.notifier_call = msm_reboot_notifier,
};
...
static int msm_restart_probe(struct platform_device *pdev)
{
	...
	if (!np) {
		dev_warn(dev, "unable to find DT imem DLOAD mode node\n");
	} else {
		dload_mode_addr = of_iomap(np, 0);
		if (!dload_mode_addr)
			dev_err(dev, "unable to map imem DLOAD offset\n");
	}

	np = of_find_compatible_node(NULL, NULL, EDL_MODE_PROP);
	if (!np) {
		dev_warn(dev, "unable to find DT imem EDLOAD mode node\n");
	} else {
		emergency_dload_mode_addr = of_iomap(np, 0);
		if (!emergency_dload_mode_addr)
			dev_err(dev, "unable to map imem EDLOAD mode offset\n");
	}

	np = of_find_compatible_node(NULL, NULL,
				"qcom,msm-imem-dload-type");
	if (!np) {
		dev_err(dev, "unable to find DT imem dload-type node\n");
		goto skip_sysfs_create;
	} else {
		dload_type_addr = of_iomap(np, 0);
		if (!dload_type_addr) {
			dev_err(dev, "unable to map imem dload-type offset\n");
			goto skip_sysfs_create;
		}
	}
...
skip_sysfs_create:
	...
	poweroff_as_halt = of_property_read_bool(dev->of_node, "qcom,poweroff-as-halt");
	if (poweroff_as_halt) {
		register_reboot_notifier(&msm_reboot_nb);
	} else {
		pm_power_off = do_msm_poweroff;
		pm_power_off = do_msm_poweroff_warm;
	}
