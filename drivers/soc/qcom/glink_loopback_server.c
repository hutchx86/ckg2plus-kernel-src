...
static inline void glink_ipc_log_init(void)
{
	glink_lbsrv_log_ctx = ipc_log_context_create(GLINK_LBSRV_NUM_LOG_PAGES,
							"glink_lbsrv", 0);
	if (!glink_lbsrv_log_ctx)
		pr_warn("%s: unable to create log context\n", __func__);
}

#else
#define GLINK_LBSRV_IPC_LOG_STR(...)
#define glink_ipc_log_init(...)
#endif
