...
static inline void msm_smd_ipc_log_init(void)
{
	smd_log_ctx = ipc_log_context_create(NUM_LOG_PAGES, "smd", 0);
	if (!smd_log_ctx) {
		pr_warn("%s: unable to create SMD logging context\n", __func__);
		msm_smd_debug_mask = 0;
	}

	smsm_log_ctx = ipc_log_context_create(NUM_LOG_PAGES, "smsm", 0);
	if (!smsm_log_ctx) {
		pr_warn("%s: unable to create SMSM logging context\n", __func__);
		msm_smd_debug_mask = 0;
	}
}
...
static void smd_channel_probe_now(struct remote_proc_info *r_info)
{
	...
	if (!shared) {
		pr_warn("%s: allocation table %d not initialized\n", __func__, r_info->remote_pid);
		return;
	}
