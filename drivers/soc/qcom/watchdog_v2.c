...
static int msm_watchdog_reboot_notifier(struct notifier_block *nb,
			       unsigned long code, void *unused)
{
	struct msm_watchdog_data *wdog_dd = container_of(nb,
		 struct msm_watchdog_data, reboot_notifier);

	mutex_lock(&wdog_dd->disable_lock);
	if (enable && ((code == SYS_HALT) || (code == SYS_POWER_OFF))) {
		wdog_disable(wdog_dd);
	}
	mutex_unlock(&wdog_dd->disable_lock);
...
static void init_watchdog_data(struct msm_watchdog_data *wdog_dd)
{
	...
	wdog_dd->reboot_notifier.notifier_call = msm_watchdog_reboot_notifier;
	register_reboot_notifier(&wdog_dd->reboot_notifier);
