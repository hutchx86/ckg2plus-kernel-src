...
static int dwc3_msm_prepare_suspend(struct dwc3_msm *mdwc)
{
	...
	dev_warn(mdwc->dev, "preventing suspend..\n");
	return -EBUSY;
...
static int dwc3_msm_probe(struct platform_device *pdev)
{
	...
	ret = of_property_read_u32(node, "qcom,current_max", &mdwc->current_max);
	if (ret) {
		dev_dbg(&pdev->dev, "setting current_max to zero.\n");
		mdwc->current_max = 0;
	}
	...
	if (of_get_property(pdev->dev.of_node, "qcom,usb-dbm", NULL)) {
		...
		if (IS_ERR(mdwc->dbm)) {
			dev_warn(&pdev->dev, "unable to get dbm device\n");
			ret = -EPROBE_DEFER;
			goto err;
		}
		/*
